...
class SampleLoader:
    ...
    def load(sample_type, samples_path, target_samples_path=None, person_id_mode=False):
        ...
        if            sample_type == SampleType.IMAGE:
            if  datas[sample_type] is None:
                datas[sample_type] = [ Sample(filename=filename) for filename in io.progress_bar_generator( Path_utils.get_image_paths(samples_path), "加载") ]
        elif          sample_type == SampleType.FACE:
            if  datas[sample_type] is None:
                if person_id_mode:
                    dir_names = Path_utils.get_all_dir_names(samples_path)
                    all_samples = []
                    for i, dir_name in io.progress_bar_generator( [*enumerate(dir_names)] , "加载"):
    ...
    def upgradeToFaceSamples ( samples, silent=False ):
        ...
        for s in (samples if silent else io.progress_bar_generator(samples, "加载")):
            ...
            try:
                ...
                if dflimg is None:
                    print ("%s 不是训练所需的dfl图像文件" % (s_filename_path.name) )
                    continue
                ...
                print ("无法加载 %s , 错误: %s" % (str(s_filename_path), traceback.format_exc() ) )
    ...
    def upgradeToFaceYawSortedSamples( samples ):
        ...
        for i in io.progress_bar_generator(range(gradations), "排序"):
