...
class SAEv2Model(ModelBase):
    ...
    def onInitialize(self):
        ...
        class SAEDFModel(object):
            def __init__(self, batch_size, resolution, ae_dims, e_ch_dims, d_ch_dims, learn_mask):
                ...
                self.src_dst_pyrest_trainable_weights = self.encoder.trainable_weights + self.decoder.trainable_weights + self.pyrest.trainable_weights
                ...
                self.src_pyr = Input ( (3,) )
                self.dst_pyr = Input ( (3,) )
                ...
                self.pred_src_src = self.decoder( K.concatenate ( [ self.src_code, src_label, self.src_pyr]) )#, K.concatenate ( [self.src_bg_code, src_label])] )
                self.pred_dst_dst = self.decoder( K.concatenate ( [ self.dst_code, dst_label, self.dst_pyr]) )#, K.concatenate ( [self.dst_bg_code, dst_label])] )
                self.pred_src_dst = self.decoder( K.concatenate ( [ self.dst_code, src_label, self.dst_pyr]) )#, K.concatenate ( [self.dst_bg_code, dst_label])] )
                
                self.pred_dst_src_pyr = self.decoder( K.concatenate ( [ self.dst_code, dst_label, self.src_pyr]) )#, K.concatenate ( [self.dst_bg_code, dst_label])] )
        ...
        if self.is_training_mode:
            ...
            self.src_dst_train = K.function ([self.model.warped_src, 
                                              ...
                                              [src_loss,dst_loss],
                                              self.src_dst_opt.get_updates( [src_loss+dst_loss]+opt_D_loss, self.model.src_dst_pyrest_trainable_weights)
                                             )
            ...
            self.src_dst_pyr_train = K.function ([self.model.warped_dst, self.model.src_pyr],
                                              [src_dst_pyr_loss],
                                              self.src_dst_opt.get_updates( [src_dst_pyr_loss], self.model.src_dst_trainable_weights)
                                             )
    ...
    def get_ConverterConfig(self):
        ...
        return self.predictor_func, (self.options['resolution'], self.options['resolution'], 3), converters.ConverterConfigMasked(face_type=face_type,
                                     default_mode = 'overlay' if self.options['apply_random_ct'] or self.options['face_style_power'] or self.options['bg_style_power'] else 'seamless',
