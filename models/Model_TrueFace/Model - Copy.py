...
class SAEv2Model(ModelBase):
    ...
    def onInitialize(self):
        ...
        class SAEDFModel(object):
            def __init__(self, batch_size, resolution, ae_dims, e_ch_dims, d_ch_dims, learn_mask):
                ...
                self.src_dst_trainable_weights = self.encoder.trainable_weights + self.decoder.trainable_weights
                ...
                src_label = K.tile(K.constant ( [[1,0]], dtype=K.floatx() ), (K.shape(self.src_code)[0] , 1))
                dst_label = K.tile(K.constant ( [[0,1]], dtype=K.floatx() ), (K.shape(self.dst_code)[0] , 1))
                #import code
                ...
                self.pred_src_src = self.decoder( K.concatenate ( [self.src_code, src_label]) )#, K.concatenate ( [self.src_bg_code, src_label])] )
                self.pred_dst_dst = self.decoder( K.concatenate ( [self.dst_code, dst_label]) )#, K.concatenate ( [self.dst_bg_code, dst_label])] )
                self.pred_src_dst = self.decoder( K.concatenate ( [self.dst_code, src_label]) )#, K.concatenate ( [self.dst_bg_code, dst_label])] )
        ...
        if self.is_training_mode:
            ...
            if self.true_face_training:
                ...
                self.D_train = K.function ([self.model.warped_src, self.model.warped_srcm, self.model.warped_dst, self.model.warped_dstm],[loss_D], self.D_opt.get_updates(loss_D, self.dis.trainable_weights) )

            self.src_dst_train = K.function ([self.model.warped_src, self.model.warped_srcm, 
                                              self.model.warped_dst, self.model.warped_dstm,  self.model.target_src, self.model.target_srcm, self.model.target_dst, self.model.target_dstm],
                                             [src_loss,dst_loss],
            ...
            #                                 [src_loss],
            #                                 self.src_dst_opt.get_updates( [src_loss]+opt_D_loss, self.model.src_dst_trainable_weights)
            #                                 )
            ...
            if self.options['learn_mask']:
                ...
                self.AE_view = K.function ([self.model.warped_src, self.model.warped_srcm, self.model.warped_dst, self.model.warped_dstm], [self.model.pred_src_src, self.model.pred_dst_dst, self.model.pred_src_dst ])
    ...
    def onTrainOneIter(self, generators_samples, generators_list):
        warped_src, warped_srcm, target_src, target_srcm = generators_samples[0]
        warped_dst, warped_dstm, target_dst, target_dstm = generators_samples[1]

        feed = [warped_src, warped_srcm, warped_dst, warped_dstm, target_src, target_srcm, target_dst, target_dstm]

        src_loss, dst_loss, = self.src_dst_train (feed)

        if self.true_face_training:
            self.D_train([warped_src, warped_srcm, warped_dst, warped_dstm])
    ...
    def onGetPreview(self, sample):
        test_S   = sample[0][2][0:4] #first 4 samples
        test_S_m = sample[0][3][0:4] #first 4 samples
        test_D   = sample[1][2][0:4]
        ...
        if self.options['learn_mask']:
            ...
            S, D, SS, DD, SD, = [ np.clip(x, 0.0, 1.0) for x in ([test_S,test_D] + self.AE_view ([test_S, test_S_m, test_D, test_D_m]) ) ]
