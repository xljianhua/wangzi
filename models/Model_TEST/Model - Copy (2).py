...
class SAEHDModel(ModelBase):
    ...
    def onInitializeOptions(self, is_first_run, ask_override):
        ...
        if is_first_run or ask_override:
            ...
            self.options['ct_mode'] = io.input_str ("颜色转换模式适用于src脸部设置。( none/rct/lct/mkl/idt, 帮助:? 跳过:{default_ct_mode}) : ", default_ct_mode, ['none','rct','lct','mkl','idt'], help_message="更改drc样本附近的src样本的颜色分布。尝试所有模式以找到最佳的颜色。")
    ...
    def onInitialize(self):
            ...
            def __init__(self, resolution, ae_dims, e_ch_dims, d_ch_dims, learn_mask):
                ...
                def dec_flow(output_nc, d_ch_dims, is_mask=False):
                    ...
                    def func(x):
                        ...
                        return Conv2D(3, kernel_size=5, padding='same', activation='sigmoid')(x), \
                               Conv2D(1, kernel_size=5, padding='same', activation='sigmoid')(x)
        ...
        if self.is_training_mode:
            ...
            target_src_masked_opt = self.model.warped_src*(1-self.model.pred_src_srcm)        
                
            #pred_src_src_masked_opt = self.model.pred_src_src*self.model.pred_src_srcm + self.model.warped_src*(1-self.model.pred_src_srcm)
            pred_src_src_masked_opt = self.model.pred_src_src*self.model.pred_src_srcm
            
            target_dst_masked_opt = self.model.warped_dst*(1-self.model.pred_dst_dstm)                   
            pred_dst_dst_masked_opt = self.model.pred_dst_dst*self.model.pred_dst_dstm
    ...
    def onGetPreview(self, sample):
        ...
        st_m = []
        for i in range(len(test_S)):
            ar = S[i]*test_S_m[i], SSM[i], D[i]*test_D_m[i], DDM[i], SDM[i]
            st_m.append ( np.concatenate ( ar, axis=1) )

        result += [ ('SAEHD mask', np.concatenate (st_m, axis=0 )), ]
