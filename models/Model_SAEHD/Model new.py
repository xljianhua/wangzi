...
class SAEv2Model(ModelBase):
    ...
    def onInitializeOptions(self, is_first_run, ask_override):
        ...
        if is_first_run or ask_override:
            ...
            self.options['ct_mode'] = io.input_str (f"学习颜色转换模式? ( none/hue/lct, ?:help skip:{default_ct_mode}) : ", default_ct_mode, help_message="通过应用随机dst样本的LCT颜色转移来增加src样本的变化。就像【面部风格样式】学习，但更精确的颜色转移，没有模型崩溃的风险，也不需要额外的GPU资源，但培训时间可能 更长，因为src的面貌变得更加多样化。")
    ...
    def onInitialize(self):
        ...
        apply_random_ct = self.options.get('ct_mode', 'none') == 'lct'
        ...
        if self.is_training_mode:
            ...
            t_mode_bgr = t.MODE_BGR_RANDOM_HUE_SHIFT if (self.options['ct_mode'] == 'hue' or self.pretrain) else t.MODE_BGR
            ...
            t_img_warped = t.IMG_TRANSFORMED
            ...
            self.set_training_data_generators ([
                    SampleGeneratorFace(training_data_src_path, sort_by_yaw_target_samples_path=training_data_dst_path if sort_by_yaw else None,
                        ...
                        output_sample_types = [ {'types' : (t_img_warped, face_type, t_mode_bgr), 'resolution':resolution, 'apply_ct': apply_random_ct},
                                                ...
                                                {'types' : (t.IMG_TRANSFORMED, face_type, t.MODE_BGR), 'resolution': resolution, 'apply_ct': apply_random_ct },
    ...
    def onTrainOneIter(self, generators_samples, generators_list):
        warped_src, target_src, target_src_gt, target_srcm = generators_samples[0]
        warped_dst, target_dst, target_dst_gt, target_dstm = generators_samples[1]
